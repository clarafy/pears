...
def data():
    ...
    return PairwiseComparisonData(observations, items)
...
@pytest.fixture
def fitted_model_with_bootstrap_cis(data):
    model = BradleyTerryModel()
    model.fit(data, ci_method="bootstrap", n_bootstrap=100)
    return model
...
class TestBradleyTerryModel:
    ...
    def test_fit_and_scores_correctness(self, fitted_model):
        """Test fit() and scores() with exact expected values on dummy data."""
        # Get scores
    ...
    def test_fit_alpha_validation(self, data):
        model = BradleyTerryModel()
        with pytest.raises(ValueError):
            model.fit(data, alpha=1.5)
        with pytest.raises(ValueError):
            model.fit(data, alpha=-0.1)

    def test_method_validation(self, data):
        """Test that invalid method raises ValueError."""
        model = BradleyTerryModel()
        with pytest.raises(ValueError, match="method must be"):
            model.fit(data, ci_method="invalid")

    def test_bootstrap_reproducibility(self, data):
        """Test that bootstrap with seed produces reproducible results."""
        model = BradleyTerryModel()
        model.fit(data, ci_method="bootstrap", alpha=0.05, n_bootstrap=5, seed=42)
        model2 = BradleyTerryModel()
        model2.fit(data, ci_method="bootstrap", alpha=0.05, n_bootstrap=5, seed=42)

        cis1 = model.confidence_intervals()
        cis2 = model2.confidence_intervals()
        assert cis1.keys() == cis2.keys()

        # Results should be identical with same seed
        for item in cis1:
            assert cis1[item][0] == pytest.approx(cis2[item][0])
            assert cis1[item][1] == pytest.approx(cis2[item][1])

    def test_sandwich_cis_contains_point_estimate(self, fitted_model):
        """Test that point estimates fall within CIs for both methods."""
        scores = fitted_model.scores()
        cis_sandwich = fitted_model.confidence_intervals()
        for label in scores:
            lower, upper = cis_sandwich[label]
            assert lower - 1e-6 <= scores[label] <= upper + 1e-6

    def test_bootstrap_cis_contains_point_estimate(self, fitted_model_with_bootstrap_cis):
        """Test that point estimates fall within CIs for bootstrap method."""
        scores = fitted_model_with_bootstrap_cis.scores()
        cis_bootstrap = fitted_model_with_bootstrap_cis.confidence_intervals()
        for label in scores:
